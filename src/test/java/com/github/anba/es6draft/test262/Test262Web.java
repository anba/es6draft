...
public final class Test262Web {
    ...
    @ClassRule
    public static TestRealms<Test262Info> realms = new TestRealms<Test262Info>(configuration, Test262RealmData::new) {
        @Override
        protected BiConsumer<ScriptObject, ModuleRecord> getImportMeta() {
            return (meta, module) -> {
                // Change the prototype of the `import.meta` object to ensure no test262 tests are created that test the
                // prototype is always `null` (the default value).
                meta.setPrototypeOf(module.getRealm().defaultContext(), new OrdinaryObject(module.getRealm()));
            };
        }
    };
