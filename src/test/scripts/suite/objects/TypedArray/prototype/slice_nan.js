...
// Float32 -> Float32
{
  ...
  assertSame(len, rf32.length);
  assertSame(len, ri32.length);

  // Same bits.
  for (let i = 0; i < len; ++i) {
    assertSame(NaNs.Float32[i], ri32[i]);
  }
}
...
// Float32 -> Float64
{
  ...
  assertSame(len, rf64.length);
  assertSame(2 * len, ri32.length);

  // NaN bits canonicalized.
  for (let i = 0; i < len; ++i) {
    assertEquals(cNaN.Float64, geti64(ri32, i));
  }
}
...
// Float64 -> Float64
{
  ...
  // Same bits.
  for (let i = 0; i < len; ++i) {
    assertEquals(NaNs.Float64[i], geti64(ri32, i));
  }
}
...
// Float64 -> Float32
{
  ...
  // NaN bits canonicalized.
  for (let i = 0; i < len; ++i) {
    assertSame(cNaN.Float32, ri32[i]);
  }
}
