...
with (obj) {
  (1,eval)("function f1() { return [w, x, y, z] }");
}
...
// [[Scope]] for f1 does not include `obj` lexical environment
assertSame("global-w", f1()[0]);
assertSame("global-x", f1()[1]);
assertSame("global-y", f1()[2]);
assertSame("global-z", f1()[3]);


// WithStatement without inner BlockStatement
with (obj) (1,eval)("function f2() { return [w, x, y, z] }");
...
// [[Scope]] for f2 does not include `obj` lexical environment
assertSame("global-w", f2()[0]);
assertSame("global-x", f2()[1]);
...
try { throw obj } catch ({w, x}) {
  (1,eval)("function f3() { return [w, x, y, z] }");
}
...
// [[Scope]] for f3 does not include `obj` lexical environment
assertSame("global-w", f3()[0]);
assertSame("global-x", f3()[1]);
...
// BlockStatement
{
  var w = "local-w";
  let x = "local-x";
  (1,eval)("function f4() { return [w, x, y, z] }");
}
...
// [[Scope]] for f4 does not include `obj` lexical environment
assertSame("local-w", f4()[0]);
assertSame("global-x", f4()[1]);
