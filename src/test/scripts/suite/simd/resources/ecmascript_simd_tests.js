...
function simdToLocaleString(type, value) {
  ...
  for (var i = 1; i < type.lanes; i++) {
    str += sep + type.fn.extractLane(value, i).toLocaleString();
  }
  return str + ")";
}
...
function testReplaceLane(type) {
  ...
  // testIndexCheck(null); // FIXME: invalid check ToNumber(null) === ToLength(null)
  testIndexCheck(undefined);
  testIndexCheck({});
  // testIndexCheck(true); // FIXME: invalid check
  testIndexCheck('yo');
  testIndexCheck(-1);
  testIndexCheck(128);
}
...
function testFrom(toType, fromType, name) {
  ...
  for (var v of fromType.interestingValues) {
    ...
    if (toType.minVal !== undefined &&
        (v < toType.minVal || v > toType.maxVal || v !== v)) {
