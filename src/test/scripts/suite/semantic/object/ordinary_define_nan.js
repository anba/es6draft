...
function testDefine(description, object, propertyKey) {
...
for (let k in objects) {
  if (k === "mappedArguments") {
    propertyKeys.filter(p => (p !== 0))
                .forEach(p => testDefine(k, objects[k](), p));
    testDefineMappedArguments(k, objects[k](), 0);
  } else {
    propertyKeys.forEach(p => testDefine(k, objects[k](), p));
  }
}
